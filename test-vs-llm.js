// Simple test script for the VS LLM provider
...
const testVSLLM = async () => {
  try {
    ...
    const boltResponse = await fetch('http://localhost:5174/api/llmcall', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (boltResponse.ok) {
      ...
      console.log(`   Content: "${boltResult.choices[0].message.content}"`);
